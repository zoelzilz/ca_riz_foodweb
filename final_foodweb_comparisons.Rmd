...
sum.out.np <- sum(out.np)
n.out.np <- length(out.np)
sd_gen.np <-sd(out.np)
...
parasite <- round(c(m, link_dens.p, connectance, longest.p, average_chain_length_p, mean_deg.p, sd_deg.p, mean_gen.p, sd_gen.p, mean_vul.p, sd_vul.p), digits = 4)
free_living <- round(c(m.np, link_dens.np, connectance.np, longest.np, average_chain_length.np, mean_deg.np, sd_deg.np, mean_gen.np, sd_gen.np, mean_vul.np, sd_vul.np), digits = 4)
...
#### No parasite web first
```{r null model no parasites}
...
model.list.noparasites <- list() #empty lsit
...
for(i in 1:1000) {
  matrix <- make_NM(size = num_nodes, 
                    ...
                    net_kind = "matrix", 
                    connectance = connectance.np)
  model.list.noparasites[[i]] <- matrix
}
...
igraphs.list <- lapply(model.list.noparasites, graph.adjacency, mode = "directed") 

# runs fine as a standalone but once passed into a for loop it throws a warning that NAs are produced... ignoring for now?

# save both igraph objects and normal (takes longer than just running the code honestly)
#save(model.list.noparasites, file = "nopsite_web_null_matrices.rda")
#save(igraphs.list, file = "nopsite_web_null_igraphs.rda")

```

#### Repeat for parasite web
```{r null model parasites}

p_matrix <- as.matrix(as_adjacency_matrix(igraph_psites)) # not sure why we need two layers to coerce to matrix byt whatever
...
# netcom is for comparing networks to each other or to null models, so can make null models using niche "mechanisms":
niche_num.p = stats::runif(n = num_nodes)
nullweb.p <- make_NM(size = num_nodes,
                      niches = niche_num.p,
                      net_kind = "matrix",
...
matrix.p <- matrix() # empty matrix
model.list.parasites <- list() #empty lsit

for(i in 1:1000) {
  matrix.p <- make_NM(size = num_nodes, 
                    niches= niche_num.p, 
                    net_kind = "matrix", 
                    connectance = connectance)
  model.list.parasites[[i]] <- matrix.p
}


# need to make them all igraph objects for the next step, using lapply

igraphs.list.p <- lapply(model.list.parasites, graph.adjacency, mode = "directed") 
...
### No parasites first
#### Mean and SD degree
```{r mean and SD degree for no parasite null web}
# test out degree on just one igraph obj from the list

mean(igraph::degree(igraphs.list[[1]], mode = "all")) # 1.6
mean(igraph::degree(igraph_psites, mode = "all")) # 14, normal (high tho)
mean(igraph::degree(igraph_nullweb.np)) # ok this is 2, normal

######## mean degree forloop ########

mean.deg <- vector() # empty vector for "named numbers" (structure of the deg objects we already created)
degrees.list <- list() # empty list

for(i in 1:1000) {
  mean.deg <- mean(igraph::degree(igraphs.list[[i]], mode = "all"))
  degrees.list[[i]] <- mean.deg
}

######## SD degree forloop ########

sd.deg <- vector() # empty vector for "named numbers" (structure of the deg objects we already created)
sd.degrees.list <- list() # empty list

for(i in 1:1000) {
  sd.deg <- sd(igraph::degree(igraphs.list[[i]], mode = "all"))
  sd.degrees.list[[i]] <- sd.deg
}


############################################
########## MEANS and MODEL ERRORS ##########
############################################

######## model error mean degree ########
degrees.np <- degrees.list %>% 
  unlist()

# visualize distribution
deg.dist.np <- degrees.np %>% 
  as_tibble() %>% 
# Density curve
...
# get 95% CI
ci.deg.np <- degrees.np %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.deg.np <- degrees.np
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.deg.np <- med.deg.np - (mean(degrees.np) - ci.deg.np) # minus bc median is less than empirical
  
ME.deg.np.nulls <- (med.deg.np - mean_deg.np)*norm.deg.np # i hope i'm doing this right

########## model error sd degree #########
sd.degrees.np <- sd.degrees.list %>% 
  unlist()

# visualize distribution
sddeg.dist.np <- sd.degrees.np %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# 2 tailed
...
# get 95% CI
ci.sd.deg.np <- sd.degrees.np %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.sd.deg.np <- sd.degrees.np %>% 
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.sd.deg.np <- med.sd.deg.np - (mean(sd.degrees.np) - ci.sd.deg.np) # minus bc median is less than empirical

ME.degsd.np.nulls <- (med.sd.deg.np - sd_deg.np)*norm.sd.deg.np


```

#### SD Generality and Vulnerability
```{r sd generality and vulnerability for no parasite null web}

######## SD generality forloop ########

sd.gen <- vector() # empty vector for "named numbers" (structure of the deg objects we already created)
gen.list <- list() # empty list

for(i in 1:1000) {
  sd.gen <- sd(igraph::degree(igraphs.list[[i]], mode = "out"))
  gen.list[[i]] <- sd.gen
}

######## SD vulnerability forloop ########
sd.vul <- vector() # empty vector for "named numbers" (structure of the deg objects we already created)
vul.list <- list() # empty list

for(i in 1:1000) {
  sd.vul <- sd(igraph::degree(igraphs.list[[i]], mode = "in"))
  vul.list[[i]] <- sd.vul
}
...
####### model error SD generality #######
generality.np <- gen.list %>% 
  unlist() 

# visualize distribution
gen.dist.np <- generality.np %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# 2 tailed
...
# get 95% CI
ci.gen.np <- generality.np %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.gen.np <- generality.np %>% 
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.gen.np <- med.gen.np - (mean(generality.np) - ci.gen.np) # minus bc median is less than empirical

ME.gen.np.nulls <- (med.gen.np - sd_gen.np)*norm.gen.np

#######  model error SD vulnerability ####### 
vulnerability.np <- vul.list %>% 
  unlist() 

# visualize distribution
vul.dist.np <- vulnerability.np %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# 2 tailed
...
# get 95% CI
ci.vul.np <- vulnerability.np %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.vul.np <- vulnerability.np %>% 
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.vul.np <- med.vul.np - (mean(vulnerability.np) - ci.vul.np) # minus bc median is less than empirical

ME.vul.np.nulls <- (med.vul.np - sd_vul.np)*norm.vul.np

```

#### Longest Chain (easy)
```{r mean longest chain for no parasite null web}

######## longest chain ######## 
longest.np.nulls <- lapply(igraphs.list, diameter) %>% 
  unlist() # need to first extract the vector of values from the list using unlist

##### model error ##### 
# visualize distribution
longest.dist.np <- longest.np.nulls %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# i think this is 1 tailed?

# get 95% CI
ci.longest.np <- longest.np.nulls %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.longest.np <- longest.np.nulls %>% 
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.longest.np <- (mean(longest.np.nulls) + ci.longest.np) - med.longest.np # plus CI and then minut the median??? bc distribution is 1 tailed to the right (+)

# model error
ME.longest.np.nulls <- (med.longest.np - longest.np)*norm.longest.np
...
#### Mean and SD degree
```{r mean and SD degree for parasite null web}

######## mean degree forloop ########

mean.deg.p <- vector() # empty vector for "named numbers" (structure of the deg objects we already created)
degrees.list.p <- list() # empty list

for(i in 1:1000) {
  mean.deg.p <- mean(igraph::degree(igraphs.list.p[[i]], mode = "all"))
  degrees.list.p[[i]] <- mean.deg.p
}

######## SD degree forloop ########

sd.deg.p <- vector() # empty vector for "named numbers" (structure of the deg objects we already created)
sd.degrees.list.p <- list() # empty list

for(i in 1:1000) {
  sd.deg.p <- sd(igraph::degree(igraphs.list.p[[i]], mode = "all"))
  sd.degrees.list.p[[i]] <- sd.deg.p
}
...
######## model error mean degree ########
degrees.p <- degrees.list.p %>% 
  unlist()

# visualize distribution
deg.dist.p <- degrees.p %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# 2 tailed
...
# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.deg.p <- med.deg.p - (mean(degrees.p) - ci.deg.p) # minus bc median is less than empirical
  
ME.deg.p.nulls <- (med.deg.p - mean_deg.p)*norm.deg.p # i hope i'm doing this right

########## model error sd degree #########
sd.degrees.p <- sd.degrees.list.p %>% 
  unlist()

# visualize distribution
sddeg.dist.p <- sd.degrees.p %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# 2 tailed
...
# get 95% CI
ci.sd.deg.p <- sd.degrees.p %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.sd.deg.p <- sd.degrees.p %>% 
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.sd.deg.p <- med.sd.deg.p - (mean(sd.degrees.p) - ci.sd.deg.p) # minus bc median is less than empirical

ME.degsd.p.nulls <- (med.sd.deg.p - sd_deg.p)*norm.sd.deg.p


```

#### SD Generality and Vulnerability
```{r sd generality and vulnerability for parasite null web}

######## SD generality forloop ########

sd.gen.p <- vector() # empty vector for "named numbers" (structure of the deg objects we already created)
gen.list.p <- list() # empty list

for(i in 1:1000) {
  sd.gen.p <- sd(igraph::degree(igraphs.list.p[[i]], mode = "out"))
  gen.list.p[[i]] <- sd.gen.p
}

######## SD vulnerability forloop ########
sd.vul.p <- vector() # empty vector for "named numbers" (structure of the deg objects we already created)
vul.list.p <- list() # empty list

for(i in 1:1000) {
  sd.vul.p <- sd(igraph::degree(igraphs.list.p[[i]], mode = "in"))
  vul.list.p[[i]] <- sd.vul.p
}
...
####### model error SD generality #######
generality.p <- gen.list.p %>% 
  unlist() 

# visualize distribution
gen.dist.p <- generality.p %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# 2 tailed
...
# get 95% CI
ci.gen.p <- generality.p %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.gen.p <- generality.p %>% 
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.gen.p <- med.gen.p - (mean(generality.p) - ci.gen.p) # minus bc median is less than empirical

ME.gen.p.nulls <- (med.gen.p - sd_gen.p)*norm.gen.p

#######  model error SD vulnerability ####### 
vulnerability.p <- vul.list.p %>% 
  unlist() 

# visualize distribution
vul.dist.p <- vulnerability.p %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# 2 tailed
...
# get 95% CI
ci.vul.p <- vulnerability.p %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.vul.p <- vulnerability.p %>% 
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.vul.p <- med.vul.p - (mean(vulnerability.p) - ci.vul.p) # minus bc median is less than empirical

ME.vul.p.nulls <- (med.vul.p - sd_vul.p)*norm.vul.p

```

#### Longest Chain (easy)
```{r mean longest chain for parasite null web}

######## longest chain ######## 
longest.p.nulls <- lapply(igraphs.list.p, diameter) %>% 
  unlist() # need to first extract the vector of values from the list using unlist

##### model error ##### 
# visualize distribution
longest.dist.p <- longest.p.nulls %>% 
  as_tibble() %>% 
# Density curve
  ggplot(., aes(x=value)) + geom_density()
# i think this is 1 tailed?
...
# get 95% CI
ci.longest.p <- longest.p.nulls %>% 
  sd() %>% 
  `*`(1.96) # horrible syntax for multiplying in a pipe

# calculate median
med.longest.p <- longest.p.nulls %>% 
  median()

# normalization coefficient (difference between median and upper or lower bound of 95% dist, so mean +/- 95% CI) according to Williams and Martinez 2008
norm.longest.p <- (mean(longest.p.nulls) + ci.longest.p) - med.longest.p # plus CI and then minut the median??? bc distribution is 1 tailed to the right (+)

# model error
ME.longest.p.nulls <- (med.longest.p - longest.p)*norm.longest.p
