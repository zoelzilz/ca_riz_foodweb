...
node_master <- read_csv("RIZspecieslist_fulltaxonomy_20mar24.csv")
...
```{r reading in literature search lit_inxs-analysis from google sheets}
...
lit_inxs1 <- read_sheet("https://docs.google.com/spreadsheets/d/1vFt4hnBJucTUrCGAJOWozgBq_E0ev33iyBH6u3PLjbE/edit?pli=1#gid=0", sheet = "records") %>% 
  ...
  mutate(resource_taxonomic_resolution = tolower(resource_taxonomic_resolution)) 
...
lit_inxs <- left_join(lit_inxs1, consumer_metadata, by = (join_by(focal_consumer_taxon))) %>% 
  select(reference_number, consumerNum, category, consumerStage, c_nsboth, czone, focal_consumer_taxon, focal_consumer_life_stage, consumption_type, trophic_strategy, resource_taxon, resource_taxonomic_resolution, added_to_edge_list, ep_riz) %>%  #doing this mostly to reorder things
  ...
  mutate(justification = 3) # this justification code means that interaction was reported in the literature
...
write_csv(lit_inxs_to_qc, "data/lit_search_manual_entry_consumers_20mar2024.csv") 
...
#create a subset of clean lit inxs where their resource is resolved to species
...
species_subset <- lit_inxs_to_match %>% 
  ...
  select(reference_number, consumerNum, focal_consumer_taxon, category, focal_consumer_life_stage, consumption_type, trophic_strategy, species, resource_taxonomic_resolution) %>% # selecting relevant columns that will help with QA/QC later
...
```
Looks like 1077 of 1692 total entries are resolved to species (and per next chunk, 580 entries have consumer species and resource species that match nodes in the nodes_master dataframe)
