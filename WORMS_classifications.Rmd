...
n_distinct(nodes_master$species)

# how many species below point conception and above

north_nodes <- nodes_master %>% 
  filter(n_s_both != "S")

n_distinct(north_nodes$species)

south_nodes <- nodes_master %>% 
  filter(n_s_both != "N") 

n_distinct(south_nodes$species)

both_nodes <- nodes_master %>% 
  filter(n_s_both == "Both" | n_s_both == "Central")
...
zone_barplot <- ggplot(zone_groups, aes(fill = clean_zone, x = n_s, y = count)) +
  geom_bar(position = "stack", stat = "identity")
