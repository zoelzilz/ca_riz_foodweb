...
## edges ##
edges_unclean <- read_csv("data/links_frommaster_7jun2024.csv") %>% # updated again, as above
...
edges_final <- edges_clean %>% 
  ...
  summarize(ref_num = paste0(ref_num, collapse = ", "),
            ...
            ) # decided i dont need localities, or the other columns i left out
...
```
## Now we need to code in some missing elements of columns:
1. locality: if ref = 435, 436 (MAH, LS) then = "California"
2. stage: if category = hydroid, = 5
...
4. Justification: if incl "diss" = PO, if incl "PO", PO
