...
### Let's Make it interactive so we can explore using visNetwork or networkD3
```{r visNetwork}
library(visNetwork)
library(networkD3)
...
# first going to try to plot the igraph object using visNetwork, which is apparently still experimental
#visIgraph(trophic_web_psites)
...
#visNetwork(nodes2, trophic_links2, width= "100%") %>% 
...
visNetwork(vis.nodes, vis.links, width = "2000px", height = "2000px") %>% 
  ...
  #visHierarchicalLayout() this layout crashes R
  #visIgraphLayout(layout = trophic_layout) # not currently working because our custom layout matrix is not an igraph layout
