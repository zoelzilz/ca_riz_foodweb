...
edges_unclean <- read_csv("data/links_frommaster_8oct2024.csv") %>% # updated finally in oct to include revisions AND remove unrealiable sanak links
  ...
  filter(incl.code != 0) # manually removed interaction late in the game
#View(edges_unclean)
...
nodes_attributes_clean <- all_nodes_w_attributes %>% 
  ...
  mutate(clean_stage = case_when(life_stage == "adult" ~ 1,
                                 ...
                                 life_stage == "parthenitae" ~ 3,
                                 life_stage == "redia" ~ 3,
                                 life_stage == "cercaria" ~ 3,
                                 life_stage == "sporocyst" ~ 3,
                                 ...
                                 life_stage == "juvenile" & organismal_category == "bivalve" ~ 4,
                                 ...
                                 life_stage == "juvenile" & organismal_category == "nematode" ~ 4,
                                 life_stage == "pupae" ~ 4,
                                 life_stage == "small" ~ 4,
                                 life_stage == "young" ~ 4,
...
edges_clean <- inner_join(resourceNums_keep, consumers_clean, by = join_by(resourceNum)) %>%  # we can name this one edges_clean because we're done!
  ...
  # need to clean up zones for downstream sorting!
  ...
  # rename this one col for ease of use
  rename(habitat = pools_exposed_interstitial)
```
...
5. habitat: if fish, gammarids, shrimp, nudibranch, pools
            if filter feeder, pools
...
locs <- read_csv("data/litsearch_locations.csv") %>% 
  ...
  dplyr::select(ref_num, general_location_of_study) %>% 
...
edges_cleaner <- edges_locs %>% 
  ...
  # we also (ugh, realized this 6/18) need to make sure that there are no consumers that are EGGS:
  filter(consumerStage != 2) %>%  # ok only removing 77 interactions, phew
  ...
  ###### HABITAT ######
  mutate(habitat = case_when(is.na(habitat) & consumerCat == "fish" ~ "pools",
                             is.na(habitat) & resourceCat == "fish" ~ "pools",
                             is.na(habitat) & consumerCat == "shark" ~ "pools",
                             is.na(habitat) & resourceCat == "shark" ~ "pools",
                             is.na(habitat) & resourceCat == "amphipod" ~ "pools",
                             is.na(habitat) & consumerCat == "amphipod" ~ "pools",
                             is.na(habitat) & resourceCat == "gammarid" ~ "pools",
                             is.na(habitat) & consumerCat == "gammarid" ~ "pools",
                             is.na(habitat) & resourceCat == "euphausid" ~ "pools",
                             is.na(habitat) & consumerCat == "euphausid" ~ "pools",
                             is.na(habitat) & resourceCat == "caprellid" ~ "pools",
                             is.na(habitat) & consumerCat == "caprellid" ~ "pools",
                             is.na(habitat) & resourceCat == "hermit crab" ~ "pools",
                             is.na(habitat) & consumerCat == "hermit crab" ~ "pools",
                             is.na(habitat) & resourceCat == "shrimp" ~ "pools",
                             is.na(habitat) & consumerCat == "shrimp" ~ "pools",
                             is.na(habitat) & resourceCat == "nudibranch" ~ "pools",
                             is.na(habitat) & consumerCat == "nudibranch" ~ "pools",
                             is.na(habitat) & consumerCat == "sponge" ~ "pools",
                             is.na(habitat) & consumerCat == "sea hare" ~ "pools",
                             is.na(habitat) & resourceCat == "sea hare" ~ "pools",
                             is.na(habitat) & consumerCat == "copepod" ~ "pools",
                             is.na(habitat) & resourceCat == "copepod" ~ "pools",
                             is.na(habitat) & consumerCat == "insect" ~ "exposed",
                             is.na(habitat) & resourceCat == "insect" ~ "exposed",
                             is.na(habitat) & resourceCat == "limpet" & consumerCat != "fish" ~ "exposed",
                             is.na(habitat) & consumerCat == "isopod" ~ "both",
                             is.na(habitat) & resourceCat == "isopod" & consumerCat != "fish" ~ "both",
                             is.na(habitat) & consumerCat == "polychaete" ~ "both",
                             is.na(habitat) & resourceCat == "polychaete" & consumerCat != "fish" ~ "both",
                             is.na(habitat) & consumerCat == "octopus" ~ "pools",
                             is.na(habitat) & resourceCat == "octopus" ~ "pools",
                             is.na(habitat) & consumerCat == "anemone" ~ "pools",
                             is.na(habitat) & consumerCat == "mammal" ~ "pools",
                             is.na(habitat) & resourceCat == "true crab" ~ "both",
                             is.na(habitat) & consumerCat == "true crab" ~ "both",
                             is.na(habitat) & consumerCat == "chiton" ~ "both",
                             is.na(habitat) & resourceCat == "chiton" & consumerCat != "fish" ~ "both",
                             is.na(habitat) & consumerCat == "sea star" ~ "pools",
                             is.na(habitat) & resourceCat == "sea star" ~ "both",
                             is.na(habitat) & consumerCat == "cucumber" ~ "pools",
                             is.na(habitat) & resourceCat == "cucumber" ~ "pools",
                             is.na(habitat) & consumerCat == "mussel" ~ "pools",
                             is.na(habitat) & resourceCat == "mussel" ~ "both",
                             is.na(habitat) & consumerCat == "limpet" ~ "both",
                             is.na(habitat) & consumerCat == "human" ~ "both",
                             is.na(habitat) & resourceCat == "barnacle" ~ "both",
                             is.na(habitat) & consumerCat == "barnacle" ~ "pools",
                             is.na(habitat) & resourceCat == "urchin" ~ "both",
                             is.na(habitat) & consumerCat == "urchin" ~ "pools",
                             is.na(habitat) & resourceCat == "hydroid" ~ "both",
                             is.na(habitat) & consumerCat == "hydroid" ~ "pools",
                             is.na(habitat) & resourceCat == "anomuran" ~ "both",
                             is.na(habitat) & consumerCat == "anomuran" ~ "both",
                             is.na(habitat) & resourceCat == "bryozoan" ~ "both",
                             is.na(habitat) & consumerCat == "bryozoan" ~ "pools",
                             is.na(habitat) & resourceCat == "clam" ~ "pools",
                             is.na(habitat) & consumerCat == "clam" ~ "pools",
                             is.na(habitat) & resourceCat == "ciliate" ~ "pools",
                             is.na(habitat) & consumerCat == "ciliate" ~ "pools",
                             is.na(habitat) & resourceCat == "tunicate" ~ "both",
                             is.na(habitat) & consumerCat == "tunicate" ~ "pools",
                             is.na(habitat) & resourceCat == "snail" ~ "both",
                             is.na(habitat) & consumerCat == "snail" ~ "both",
                             is.na(habitat) & consumerCat == "brittle star" ~ "pools",
                             is.na(habitat) & consumerCat == "sea spider" ~ "pools",
                             is.na(habitat) & resourceCat == "sea spider" ~ "pools",
                             is.na(habitat) & consumerCat == "nemertean" ~ "both",
                             is.na(habitat) & resourceCat == "nemertean" ~ "both",
                             is.na(habitat) & consumerCat == "scale worm" ~ "pools",
                             is.na(habitat) & resourceCat == "scale worm" ~ "both",
                             is.na(habitat) & consumerCat == "porcelain crab" ~ "pools",
                             is.na(habitat) & resourceCat == "porcelain crab" ~ "both",
                             is.na(habitat) & consumerCat == "bird" ~ "both",
                             is.na(habitat) & resourceCat == "bird" ~ "exposed",
                             is.na(habitat) & str_detect(resourceCat, "plankton") ~ "pools",
                             is.na(habitat) & consumerCat == "abalone" ~ "pools",
                             is.na(habitat) & resourceCat == "abalone" ~ "both",
                             is.na(habitat) & resourceCat == "algae" ~ "both",
                             is.na(habitat) & resourceName == "Wrack" ~ "exposed",
                             is.na(habitat) & resourceCat == "microorganism" ~ "pools",
                             
                             
                             TRUE ~ as.character(habitat)
                             )) 
  
edges_collapse <- edges_cleaner %>% 
  ## need to add one more line of code collapsing all localities/habitats/zones for each reference
  ...
  summarize(localities = paste0(localities, collapse = ";"), # important to leave out the space after semicolon, for later
            justification = min(justification), 
            confidence = min(confidence),
            ref_location_code = min(incl.code), # ADDED THIS IN POST-REVISION ON OCT 7 2024
            zone = paste0(unique(na.omit(zone)), collapse = ", "),
            habitat = paste0(unique(na.omit(habitat)), collapse = ", ")
            )



# make a nice list with all of the confidences and references collapsed
edges_final <- edges_collapse %>% 
  ...
  summarize(ref_num = paste0(ref_num, collapse = "; "), # changed from comma oct 7
            justification = min(justification),
            ref_location_code = min(ref_location_code), # ADDED THIS IN POST-REVISION ON OCT 7 2024
            zone = paste0(unique(na.omit(zone)), collapse = ", "),
            habitat = paste0(unique(na.omit(habitat)), collapse = ", "),
            localities = paste0(unique(localities), collapse = ";"), # collapse localities into one col, unique is important to remove duplicate locations, also important to leave OUT the space, for later
            confidence = min(confidence)
            ) %>% # decided i dont need the other columns i left out
  
  ##### further cleaning now that we've made it messy again:
  #mutate(localities = str_replace(localities, ",", ";")) %>%  # replace commas with semicolons to make the next line possible:
  mutate(localities = sapply(localities, function(x) paste(unique(unlist(strsplit(x, ";"))), collapse = "; "))) %>% 
  
  # also some justifications still have more than one so apply the same formula there:
  #mutate(justification = sapply(justification, function(x) paste(min(unlist(strsplit(x, ","))), collapse = ";")))
  # above works, below is cleaner:
  ungroup() %>% # not sure if necessary
  mutate(justification = parse_number(justification))#This parses the first number it finds, dropping any non-numeric characters before the first number and all characters after the first number (we're assuming lowest number is first)
  
```
## Last thing we need to do (for pub, since this requires code downstream otherwise) is make a new node list containing ONLY the clean clean cleanest list of nodes that are present in the web, and nothing else 
(e.g. removing nodes that dont have any interactions... which can be done in code but again, for the pub)

```{r final node list from final links}

# first pull out dataframe of just consumers and their IDs
consumers_from_edgelist_final <- edges_final %>% 
  dplyr::select(consumerNum, consumerName) %>% 
  rename(nodeNum = consumerNum,
         nodeName = consumerName) # column names need to match resources df

# then dataframe of just resources and their IDs
resources_from_edgelist_final <- edges_final %>% 
  dplyr::select(resourceNum, resourceName) %>% 
  rename(nodeNum = resourceNum,
         nodeName = resourceName) # column names need to match consumers df

# stack them with rbind
nodes_final <- rbind(resources_from_edgelist_final, consumers_from_edgelist_final) %>% 
  distinct(nodeNum, .keep_all = TRUE)
...
# add attributes from potential nodes list
final_nodes_w_attributes <- left_join(nodes_final, nodes_attributes_clean, by = join_by(nodeNum)) %>% 
  dplyr::select(!nodeName) %>%  # removing this column because it's actually often innacurate (old sp name, etc)
  filter(!is.na(species))
...
## We're going to take a pause to save these dataframes as CSVs for the next rmd
```{r saving current working node list and edge list}

write_csv(final_nodes_w_attributes, "data/foodweb_V8/nodes_final_8oct2024.csv")

write_csv(edges_final, "data/foodweb_V8/links_final_8oct2024.csv")

write_csv(edges_cleaner, "data/foodweb_V8/links_uncollapsed_8oct2024.csv")

```

# At this point we can move over to final_foodweb_analyses, which is where we will remove non-trophic links and make a food web for publication ---->

```{r}
############ some checks and balances e.g. why did removing eggs as consumers delete over 100 species from the web

#view(final_nodes_w_attributes_test) # with eggs as consumers
#view(final_nodes_w_attributes) # eggs as consumers removed, as it should be

#why <- anti_join(final_nodes_w_attributes_test, final_nodes_w_attributes,  by = join_by(nodeNum))

# turns out it was also filtering out NAs and I had some parasites in the node list that didn't have life stage categories
...
trophic_links1 <- edges_final %>% # swapping to "links" lingo because armand thinks it more intuitive, so fine
  ...
  mutate(parasitic = case_when(
    ...
    dplyr::select(resourceNum,consumerNum, interaction.type#, parasitic
                  )  # RESOURCE NUM HAS TO GO FIRST OR WEB IS UPSIDEDOWN (resourcenum is FROM and consumernum is TO)
  

#free_living_links <- trophic_links1 %>% 
#  filter(parasitic == "n")
...
visNetwork(vis.nodes, vis.links, width = "2000px", height = "2000px"
  ...
  visOptions(selectedBy =  "organismal_category",
